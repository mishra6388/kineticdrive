...
export function MacbookScrollDemo({ 
  ...
  return (
    <div className={`overflow-hidden dark:bg-[#0B0B0F] bg-white w-full ${className}`}>
      <MacbookScroll
        title={title}
        badge={badge ? badge : <a href="#"><Badge className="h-10 w-10 transform -rotate-12" /></a>}
        src={src}
...
export const MacbookScroll = ({
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const progress = Math.max(0, Math.min(1, 1 - (elementTop / (elementHeight - windowHeight))));
  ...
  const scaleX = 1.2 + ((isMobile ? 1 : 1.5) - 1.2) * Math.min(1, scrollYProgress / 0.3);
  const scaleY = 0.6 + ((isMobile ? 1 : 1.5) - 0.6) * Math.min(1, scrollYProgress / 0.3);
  const translate = scrollYProgress * 1500;
  
  // Handle rotation with specific breakpoints
  let rotate = -28;
  if (scrollYProgress > 0.1 && scrollYProgress <= 0.12) {
    rotate = -28;
  } else if (scrollYProgress > 0.12 && scrollYProgress <= 0.3) {
    const progress = (scrollYProgress - 0.12) / (0.3 - 0.12);
    rotate = -28 + progress * 28; // Animate from -28 to 0
  } else if (scrollYProgress > 0.3) {
    rotate = 0;
  }
  
  const textTransform = scrollYProgress * 100;
  const textOpacity = Math.max(0, 1 - scrollYProgress / 0.2);
...
export const Lid = ({
  ...
  return (
    <div className="relative [perspective:800px]">
      ...
      <div
        style={{
          transform: `perspective(800px) scaleX(${scaleX}) scaleY(${scaleY}) rotateX(${rotate}deg) translateY(${translate}px)`,
          transformStyle: "preserve-3d",
          transformOrigin: "top",
        }}
