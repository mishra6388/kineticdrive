...
import { ArrowRight, Star, Check, TrendingUp, Code2, Smartphone, Headphones, Phone } from 'lucide-react';
import { useRouter } from 'next/navigation';
import Image from 'next/image';

const highlights = [
  { icon: TrendingUp, text: 'More Leads & Enquiries' },
  { icon: Code2,      text: 'SEO-Optimized Code' },
...
export default function HeroFormSection() {
    ...
    try {
      const res = await fetch('/api/leads', {
        ...
        body: JSON.stringify({
          ...
          company: formData.company, service: 'Web Development',
          website_type: formData.website_type, budget: formData.budget,
          requirements: formData.requirements, source: 'Website', status: 'New',
        }),
      });
  ...
  return (
    <section className="relative min-h-screen bg-[#050508] overflow-hidden flex flex-col">
      ...
      {/* ── Minimal identity strip (mobile + desktop) ── */}
     <div className="relative z-20 w-full border-b border-white/5">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 py-8 sm:py-12 lg:py-0 flex-1 flex items-center">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_480px] xl:grid-cols-[1fr_520px] gap-8 xl:gap-20 items-center w-full lg:min-h-0 lg:py-16">
          ...
          <div className="text-left order-2 lg:order-1">
            ...
            >
              We design and build fast, SEO-ready, mobile-first websites that help businesses
              generate more leads, grow their brand, and stand out online — delivered in days.
            </motion.p>
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 font-medium">
                  <span className="text-white font-bold">50+ Clients</span> trust KineticDrive across India
                </p>
              </div>
            </motion.div>
          </div>
            ...
            <div className="relative">
              ...
              <div className="relative rounded-2xl border border-white/10 bg-[#0C0C15]/95 backdrop-blur-2xl overflow-hidden shadow-[0_32px_80px_rgba(0,0,0,0.6)]">
                ...
                <div className="p-6 sm:p-7">
                  ...
                  {/* Mobile-only mini trust row — sits above form title so form isn't context-free */}
                  ...
                  <div className="mb-5">
                    <h2 className="text-[1.35rem] font-bold text-white">Get a Free Consultation</h2>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-3.5">
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-1.5" htmlFor="wd-name">
                          Full Name <span className="text-amber-500">*</span>
                        </label>
                        <input id="wd-name" type="text" placeholder="John Doe"
                      ...
                      <div>
                        <label className="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-1.5" htmlFor="wd-email">
                          Email <span className="text-amber-500">*</span>
                        </label>
                        <input id="wd-email" type="email" placeholder="you@example.com"
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-1.5" htmlFor="wd-phone">
                          Phone <span className="text-amber-500">*</span>
                        </label>
                        <input id="wd-phone" type="tel" placeholder="9876543210"
                          value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value.replace(/\D/g,'').slice(0,10)})}
                          className="w-full rounded-lg border border-white/8 bg-white/[0.04] px-3.5 py-2.5 text-sm text-white placeholder-gray-700 outline-none focus:border-amber-400/60 focus:ring-1 focus:ring-amber-400/25 transition-all duration-200"
                          required />
                      </div>
                      <div>
                        <label className="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-1.5" htmlFor="wd-company">
                          Company
                        </label>
                        <input id="wd-company" type="text" placeholder="Acme Corp"
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-1.5" htmlFor="wd-type">
                          Website Type
                        </label>
                        <select id="wd-type" value={formData.website_type} onChange={e => setFormData({...formData, website_type: e.target.value})}
                      ...
                      <div>
                        <label className="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-1.5" htmlFor="wd-budget">
                          Budget
                        </label>
                        <select id="wd-budget" value={formData.budget} onChange={e => setFormData({...formData, budget: e.target.value})}
                          className="w-full rounded-lg border border-white/8 bg-[#111120] px-3.5 py-2.5 text-sm text-white outline-none focus:border-amber-400/60 focus:ring-1 focus:ring-amber-400/25 transition-all appearance-none cursor-pointer">
                    ...
                    <div>
                      <label className="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-1.5" htmlFor="wd-req">
                        Project Details
                      </label>
                      <textarea id="wd-req" rows={2} placeholder="Describe what you need (optional)..."
                        value={formData.requirements} onChange={e => setFormData({...formData, requirements: e.target.value})}
                        className="w-full rounded-lg border border-white/8 bg-white/[0.04] px-3.5 py-2.5 text-sm text-white placeholder-gray-700 outline-none focus:border-amber-400/60 focus:ring-1 focus:ring-amber-400/25 transition-all resize-none"
                      />
                    </div>
                    ...
                    >
                      ...
                      <span className="relative flex items-center justify-center gap-2">
                        {status === 'submitting' ? (
                          ...
                          <>🚀 Get Free Consultation <ArrowRight className="h-4 w-4 ml-0.5 transition-transform duration-200 group-hover:translate-x-1" /></>
                        )}
                      </span>
                    </button>

                    <div className="flex items-center justify-center gap-5 pt-0.5 text-[11px] font-medium text-gray-600">
                      <span className="flex items-center gap-1.5"><Check className="h-3 w-3 text-amber-500/80 flex-shrink-0"/>Free Consultation</span>
